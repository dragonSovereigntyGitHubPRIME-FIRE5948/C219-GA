...
$(document).ready(function () {
  ...
  function generateArrayOfObjects(numItems) {
    let array = [];
  ...
  let arrayScatterData = generateArrayOfObjects(1000);
  let arrayScatterData2 = generateArrayOfObjects(1000);

  const data = {
    datasets: [
      {
        label: 'Genre',
        data: arrayScatterData,
        backgroundColor: colorPrimary,
      },
      {
        label: 'Language',
        data: arrayScatterData2,
        backgroundColor: colorSecondary,
      }
    ]
  };
  ...
  const config__scatterGenre = {
    ...
    options: {
      ...
      plugins: {
        title: {
          display: true,
          text: 'Rating performance relative to genre and language'
        },
        legend: {
          display: false
        }
  ...
  function setPaginationPage(pageNumber) {
    ...
    var arrayItemsToShow = [];

    if (arrayIndexesPerPage.length > 0) {
      arrayItemsToShow = arrayIndexesPerPage[pageNumber];
    }
    // hide all items.
  ...
  function getBooksToLoop(books) {
    var selector = '.tab-content__item';
    if (searched) {
      selector += '.searched';
    }
    if (filtered) {
  ...
  $(".search-bar__button").on("click", function () {
    // this stores the ORIGINAL index of the element in the main array so that the correct item is shown.
    arrayIndexesPerPage = [];
    // temp array to store a set of indexes, when reach pagination limit then will push into arrayIndexesPerPage.
    var arrayTemp = [];
    // books to loop depending on conditions (are current books filtered? searched? etc)
    var booksToLoop;
    ...
    // get 'current' books.
    // booksToLoop = getBooksToLoop(booksToLoop);
    // alert(filtered)
  ...
  $("#Filter__Button").on("click", function () {
    ...
    if (arrayFilters.length > 0) {
      ...
      $.each(arrayFilters, function (index) {
        bookItemsToLoop = getBooksToLoop(bookItemsToLoop);
        alert(bookItemsToLoop.length)
        ...
        bookItemsToLoop.each(function (index) {
          ...
          // when reach last item, add array to parent to array.
          if (index >= bookItemsToLoop.length - 1) {
            arrayIndexesPerPage.push(arrayTemp);
          }
        });
      });
